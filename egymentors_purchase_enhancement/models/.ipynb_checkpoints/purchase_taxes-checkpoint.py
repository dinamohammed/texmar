...
class PurchaseOrderTaxes(models.Model):
                ...
                for tax_id in taxes:
                    ...
                    for line in order.order_line:
                        if tax_id in line.taxes_id.ids:
                            ...
                            subtotal_discounted = vals['price_subtotal'] / vals['product_qty']
                            taxes = \
                                tax.compute_all(subtotal_discounted,
                                    vals['currency_id'],
                                    vals['product_qty'], vals['product'
                                    ], vals['partner'])
                            tax_amount += sum(t.get('amount', 0.0)
                                    for t in taxes.get('taxes', []))

            taxes_dict.append({'tax_name': tax.name,
    ...
    def compute_purchase_taxes_by_group(self):
        ...
        for order in self:
            lang_env = \
                order.with_context(lang=order.partner_id.lang).env
            tax_lines = order.order_line.filtered(lambda line: \
                    line.taxes_id.ids != [])
            res = {}
            ...
            for line in tax_lines:
                for tax_id in line.taxes_id:
                    price_discounted = line.price_subtotal / line.product_qty
                    tax_dict = tax_id.compute_all(price_discounted,
                            line.currency_id, line.product_qty,
                            line.product_id,
                    ...
                    if tax_dict:
                        ...
                        res.setdefault(tax_id.tax_group_id,
                                {'base': 0.0, 'amount': 0.0})
                        res[tax_id.tax_group_id]['amount'] += price_tax
                        tax_key_add_base = \
                            tuple(order._get_tax_key_for_group_add_base(line))
                        if tax_key_add_base not in done_taxes:
                            if line.currency_id \
                                != self.company_id.currency_id:
                                amount = \
                                    self.company_id.currency_id._convert(price_tax,
                                        line.currency_id,
                                        self.company_id,
                                        fields.Date.today())
                            else:
